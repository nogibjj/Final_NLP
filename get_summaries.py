...
# PARAMS
SUMMARY_DIR = r"C:\Users\Simrun Sharma\Desktop\NLP\Final_Project\summaries"
# Summary list info
summary_list_file = (
    r"C:\Users\Simrun Sharma\Desktop\NLP\Final_Project\four_summaries.tsv"
)
...
def wrap_data(name, summary, analysis, url):
    return {"name": name, "summary": summary, "analysis": analysis, "url": url}
...
def get_summary(summary_info):
    ...
    summary_links = [
        link.get("href") for link in summary_links if "section" in link.get("href")
    ]
    ...
    for index, section in enumerate(summary_links):
        ...
        num_subsections = max(
            1, len(subsection_links.findAll("a") if subsection_links else [])
        )
        subsection_links = [
            "page/%d/" % page_ix for page_ix in range(1, num_subsections + 1)
        ]

        section_header = " ".join(
            [
                x.strip()
                for x in soup.title.string.replace(" | SparkNotes", "").split(":")[1:]
            ]
        )
        ...
        for subsection_link in subsection_links:
            ...
            try:
                soup = BeautifulSoup(
                    urllib.request.urlopen(subsection_url), "html.parser"
                )
            except URLError as err:
                print(err, "Retrying after sleep")
                time.sleep(10)
        ...
        if "Summary:" in section_text and "Analysis:" in section_text:
            ...
            summary_text = (
                " ".join(
                    [summary for summary in section_text_split if "Summary:" in summary]
                )
                .replace("Summary:", "")
                .strip()
            )
            analysis_text = (
                " ".join(
                    [
                        analysis
                        for analysis in section_text_split
        ...
        section_data = wrap_data(
            section_header, summary_text, analysis_text, section_url
        )
